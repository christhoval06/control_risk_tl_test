    ...
    <section className="grid gap-4">
      <div className="task-panel grid gap-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-lg font-bold text-slate-950 dark:text-white">Board</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400">{board.tasks.length} issues across 3 statuses</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <Button onClick={() => setIsCreateOpen(true)}>Create task</Button>
            <Button disabled={board.isLoading} onClick={() => void board.loadTasks()} variant="outline">
              Refresh
            </Button>
          </div>
        </div>
        ...
        <div
          className="grid gap-3 rounded-lg bg-slate-200/70 p-3 dark:bg-slate-950/70 lg:grid-cols-3"
