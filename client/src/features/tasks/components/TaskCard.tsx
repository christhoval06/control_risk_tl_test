...
function TaskCardComponent({ task, onDelete, onStatusChange }: TaskCardProps) {
  ...
  return (
    <article
      className="cursor-grab rounded-md border border-slate-200 bg-white p-3 shadow-sm transition hover:border-slate-300 hover:shadow-md active:cursor-grabbing dark:border-slate-800 dark:bg-slate-950 dark:hover:border-slate-700"
      draggable
      onDragStart={startDrag}
    >
      <div className="flex items-start justify-between gap-3">
        <h3 className="text-sm font-semibold leading-5 text-slate-950 dark:text-slate-100">{task.title}</h3>
        <TaskStatusBadge status={task.status} />
      </div>
      {task.description && (
        <p className="mt-2 line-clamp-3 text-sm leading-5 text-slate-600 dark:text-slate-300">{task.description}</p>
      )}
      <dl className="mt-3 grid gap-1.5 text-xs text-slate-500 dark:text-slate-400">
        <div className="flex justify-between gap-3">
          <dt className="font-semibold text-slate-700 dark:text-slate-300">Owner</dt>
          <dd className="truncate">{task.assignedTo || 'Unassigned'}</dd>
        </div>
        <div className="flex justify-between gap-3">
          <dt className="font-semibold text-slate-700 dark:text-slate-300">Due</dt>
      ...
      <div className="mt-3 flex flex-wrap gap-1.5 border-t border-slate-100 pt-3 dark:border-slate-800">
        ...
        {nextStatus && (
          <Button
            size="sm"
            variant="secondary"
            onClick={() => void onStatusChange(task, nextStatus)}
        ...
        <Button
          className="ml-auto text-red-700 hover:bg-red-50 dark:text-red-300 dark:hover:bg-red-500/10"
          size="sm"
          variant="ghost"
...
export function TaskCardSkeleton() {
  return (
    ...
    >
      ...
      <div className="mt-4 grid gap-2 border-t border-slate-100 pt-3 dark:border-slate-800">
        <div className="flex justify-between gap-3">
