...
function TaskBoardColumnComponent({ status, tasks, isLoading, onDelete, onTaskDrop, onStatusChange }: TaskBoardColumnProps) {
  ...
  const dropTask = useCallback((event: DragEvent<HTMLElement>) => {
    event.preventDefault();
    setIsDragOver(false);
  ...
  return (
    <section
      ...
      className={cn(
        'kanban-swimlane grid min-h-130 content-start gap-3 rounded-md border border-slate-200 p-3 shadow-sm transition',
        isDragOver && 'border-teal-700 bg-teal-50'
      )}
      ...
      role="region"
    >
      <header className="flex items-start justify-between gap-3 rounded-md border border-slate-200 bg-white p-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <span className={cn('h-2.5 w-2.5 rounded-full', meta.accent)} />
            <h2 className="text-xs font-bold uppercase tracking-wide text-slate-700">{meta.label}</h2>
          </div>
          <p className="mt-1 text-xs leading-5 text-slate-500">{meta.description}</p>
        </div>
        <span className="rounded-full bg-slate-200 px-2 py-0.5 text-xs font-bold text-slate-600">{tasks.length}</span>
      </header>
      <div className="grid gap-3 pb-1">
        {isLoading ? (
          skeletonCards.map((index) => <TaskCardSkeleton key={`${status}-${index}`} />)
        ) : tasks.length === 0 ? (
          <div className="flex flex-1 flex-col min-h-40 items-center rounded-md border border-dashed border-slate-300  bg-white/70 p-4 text-sm text-slate-500">
