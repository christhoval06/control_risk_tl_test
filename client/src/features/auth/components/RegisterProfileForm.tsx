...
export function RegisterProfileForm({ defaultValues, error, isSaving, onSubmit }: RegisterProfileFormProps) {
  const { register, handleSubmit, formState: { errors } } = useForm<RegisterUserRequest>({ defaultValues });
    ...
    <form onSubmit={(event) => void handleSubmit(onSubmit)(event)}>
      ...
      <div className="mt-6 grid gap-4">
        <div>
          ...
          <input id="display-name" className="form-field" {...register('displayName', { required: 'Display name is required.' })} />
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3">
        <button className="rounded-md bg-slate-950 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50" disabled={isSaving} type="submit">
          Save profile
        </button>
